        ...
        if ($fileerror === 0) {
            if ($filesize > 30000000) {
                ...
                if (empty($checkcategory)) {
                    $quizid = $_GET['quiz'];
                    $addcategory = "UPDATE  questions SET  `question_title`=?, `category_id`=?, `description`=?, `image`=? WHERE `id`=? ";
                    $checkadd = $conn->prepare($addcategory);
                    $queryadd = $checkadd->execute([$question, $category, $description,  $filenamenew, $quizid]);
                    if ($queryadd === true) {
                        echo "<script>
